...
# --- UPDATED: Define output directory for capex_per_kw plots ---
output_dir_per_kw = os.path.join(root, 'intermediate data', 'raw capex per kw plots')

# Create the output directories if they don't exist
os.makedirs(output_dir, exist_ok=True)
print(f"Total capital cost plots will be saved to: {output_dir}")
# --- UPDATED: Create the new output directory ---
os.makedirs(output_dir_per_kw, exist_ok=True)
print(f"Capex per kW plots will be saved to: {output_dir_per_kw}")
...
try:
    ...
    # ==============================================================================
    # SECOND PLOTTING ROUTINE: WEIGHTED AVERAGE CAPEX PER KW
    # ==============================================================================
    print(f"\nGenerating and saving weighted average capex per kW plots...")
    ...
    for i, plant in enumerate(unique_plants):
        ...
        ax.plot(
            plant_df_agg['report_year'],
            plant_df_agg['capex_per_mw_weighted'] / 1000,
            marker='o',
            linestyle='-'
        )
        ...
        tech_output_dir = os.path.join(output_dir_per_kw, safe_tech)
        os.makedirs(tech_output_dir, exist_ok=True)
    ...
    print("\nProcessing for capex per kW plots complete.")
