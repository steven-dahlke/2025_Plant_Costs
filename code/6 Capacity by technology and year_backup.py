...
try:
    ...
    chart_filename = 'figure_capacity_by_technology_and_year.png'
    plt.savefig(os.path.join(output_dir, chart_filename))

    print(f"\nChart successfully saved to the 'output' folder as '{chart_filename}'")

    # --- START: PROPOSED NEW CODE BLOCK ---

    # --- 8. Read and Process EIA Generator Data ---
    print("\n--------------------------------------------------")
    ...
    # **Extract year from the 'report_date' column as requested**
    # This converts the date strings to datetime objects and extracts the year
    print("Extracting year from 'report_date' column...")
    df_generators['report_year'] = pd.to_datetime(df_generators['report_date']).dt.year

    # --- 9. Prepare EIA Data for Plotting ---
    print("\nProcessing EIA generator data for new chart...")

    # Apply the same technology mapping to the new DataFrame
    ...
    # Group by the newly created 'report_year' and technology, summing capacity
    eia_capacity_data = df_gen_cleaned.groupby(['report_year', 'technology_description'])[
        'capacity_mw'].sum().unstack().fillna(0)

    # FILTER DATA FROM 2010 through 2024
    eia_capacity_data = eia_capacity_data[(eia_capacity_data.index >= 2010) & (eia_capacity_data.index <= 2024) ]

    # Ensure all technologies from the stack order are present as columns for a consistent legend
    for col in stack_order:
        if col not in eia_capacity_data.columns:
            eia_capacity_data[col] = 0

    # Reorder columns according to the predefined stack order
    eia_capacity_data = eia_capacity_data[stack_order]

    # Convert capacity from MW to GW
    eia_capacity_data = eia_capacity_data / 1000
    print("Aggregated and reordered EIA capacity data.")

    # --- 10. Generate and Save the Second Stacked Bar Chart ---
    print("Generating second stacked bar chart for EIA data...")
    ax2 = eia_capacity_data.plot(
        kind='bar',
        stacked=True,
        figsize=(16, 9),
        color=color_map,  # Use the same color map as the first chart
        width=0.85
    )
    ...
    for i, label in enumerate(ax2.get_xticklabels()):
        # Ensure label text is valid before converting to int
        if label.get_text():
            year = int(label.get_text())
            if year % 2 != 0:
                label.set_visible(False)

    # Format legend to match the first chart
    ...
    plt.tight_layout()
    eia_chart_filename = 'figure_eia_capacity_by_tech_and_year.png'
    plt.savefig(os.path.join(output_dir, eia_chart_filename))

    print(f"\nEIA capacity chart successfully saved as '{eia_chart_filename}'")
